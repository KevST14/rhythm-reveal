...
export default function Home() {
  ...
  // Debugging session and access token
  useEffect(() => {
    if (session) {
      ...
      const fetchData = async () => {
        try {
          ...
          console.log("Top Tracks response:", topTracksRes);

          const topArtistsRes = await axios.get("https://api.spotify.com/v1/me/top/artists", {
            headers: { Authorization: `Bearer ${session.accessToken}` },
          });
          console.log("Top Artists response:", topArtistsRes);
          ...
          // Debugging data being set in state
          console.log("Playlists:", playlistRes.data.items);
          console.log("Top Tracks:", topTracksRes.data.items);
          console.log("Top Artists:", topArtistsRes.data.items);
          ...
          console.error("Error fetching Spotify data", err.response ? err.response.data : err);
        }
      };
            ...
            <section id="top-tracks" className="mt-8">
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-6 mt-4">
                {topTracks.map((track) => {
                  ...
                  return <PlaylistCard key={track.id} playlist={track} />;
                })}
              </div>
            </section>
